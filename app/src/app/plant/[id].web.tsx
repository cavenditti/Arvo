// OWNER: fe-plant-detail — Web/portal per-plant detail (FR-P-052). Same data and helpers as the
// native [id].tsx, laid out for the 1280px portal scroll area (PortalShell): breadcrumb + hero,
// then two columns — series / growth curve / flight history / scouting on the left, identity,
...
// Columns of the flight-history table (the rest live in the readings tiles).
const HISTORY_METRICS: PlantMetric[] = ['ndvi', 'ndre', 'canopy_m2', 'height_m'];
...
export default function PlantDetailWebScreen() {
  ...
  if (plantQ.isLoading) {
    ...
    const parcelName = parcelQ.data?.name ?? t('plant.parcel');
    const metaLine = [
      t(`plant.unit.${p.unit_type}`),
      p.variety,
      p.rootstock,
      p.block_name ? `${t('plant.block')} ${p.block_name}` : null,
      p.row_name ? `${t('plant.row')} ${p.row_name}` : null,
      p.planted_on ? format(parseISO(p.planted_on), 'd MMM yyyy', { locale }) : null,
    ]
      .filter(Boolean)
      .join(' · ');

    const askStatus = (next: PlantStatus) =>
      confirmDestructive({
        title: t('plant.status_change_title'),
        message: t('plant.status_change_confirm', {
          status: t(`plant.status.${next}`),
          plant: name,
        }),
      ...
      <View style={styles.page}>
        ...
        <View style={styles.titleBlock}>
          <View style={styles.titleLeft}>
            ...
            {p.external_ref ? (
              <MonoLabel>{`${t('plant.external_ref')} · ${p.external_ref}`}</MonoLabel>
            ) : null}
          </View>
          <GlyphCard
            gradient={tone.grad}
        ...
        <View style={styles.grid}>
          ...
          <View style={styles.colLeft}>
            ...
            <View style={styles.card}>
              <View style={styles.cardHead}>
                <Text style={styles.cardTitle}>{t('plant.series')}</Text>
                <MonoLabel>{t('plant.select_metric')}</MonoLabel>
              </View>
              ...
              {seriesQ.isLoading ? (
                <View style={[styles.chartEmpty, { height: 300 }]}>
                  <ActivityIndicator color={colors.primary} />
                </View>
              ) : (
                <PlantChart
            ...
            <View style={styles.card}>
              <View style={styles.cardHead}>
                <View style={styles.flex1}>
                  <Text style={styles.cardTitle}>{t(metricLabelKey(growth))}</Text>
              ...
              {growthQ.isLoading ? (
                <View style={[styles.chartEmpty, { height: 240 }]}>
                  <ActivityIndicator color={colors.primary} />
                </View>
              ) : (
                <PlantChart
                  series={growthQ.data?.series ?? []}
                  metric={growth}
                  locale={locale}
                  height={240}
                />
            ...
            <View style={styles.card}>
              <View style={styles.cardHead}>
                <Text style={styles.cardTitle}>{t('plant.history')}</Text>
              ...
              {capturesQ.isLoading ? (
                ...
                <>
                  <View style={styles.tableHead}>
                    ...
                    <MonoLabel style={styles.colValue}>{t('plant.quality')}</MonoLabel>
                  ...
                  {captures.map((c) => (
                    <View key={c.capture_id} style={styles.tableRow}>
                      <MonoValue size={13} style={styles.colDate}>
                        {format(parseISO(c.captured_at), 'd MMM yyyy', { locale })}
                      </MonoValue>
                      <MonoValue size={13} weight="400" style={styles.colValue}>
                        {c.quality == null ? '—' : `${c.quality}%`}
                      </MonoValue>
                      {HISTORY_METRICS.map((m) => (
          ...
          <View style={styles.colRight}>
            ...
            <View style={styles.card}>
              <View style={styles.cardHead}>
                <Text style={styles.cardTitle}>{t('plant.identity')}</Text>
              </View>
              <View style={styles.fields}>
                ...
                />
                <Field
                  label={t('plant.created_at')}
                  value={format(parseISO(p.created_at), 'd MMM yyyy', { locale })}
                  mono
                />
                <Field
            ...
            <View style={styles.card}>
              <View style={styles.cardHead}>
                <Text style={styles.cardTitle}>{t('plant.latest_metrics')}</Text>
              </View>
              {latestQ.isLoading ? (
                <ActivityIndicator color={colors.primary} style={styles.pad} />
              ) : headline ? (
            ...
            <View style={styles.card}>
              <View style={styles.cardHead}>
                <Text style={styles.cardTitle}>{t('plant.alerts')}</Text>
              </View>
              {alertsQ.isLoading ? (
                <ActivityIndicator color={colors.primary} style={styles.pad} />
              ) : alerts.length === 0 ? (
            ...
            {TRANSITIONS[p.status].length > 0 ? (
              <View style={styles.card}>
                <View style={styles.cardHead}>
                  <Text style={styles.cardTitle}>{t('plant.actions')}</Text>
                </View>
...
function ObsRow({ o, locale, token }: { o: Observation; locale: Locale; token: string | null }) {
  const { t } = useTranslation();
...
const styles = StyleSheet.create({
  ...
  ctaBtnTxt: { fontSize: 13, fontFamily: fonts.bodyBold, color: colors.onPrimary },
  ...
  field: { flexGrow: 1, flexBasis: 120, gap: 2 },
